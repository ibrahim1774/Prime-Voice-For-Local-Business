...
const FIELDS = [
  ...
  { key: "businessLink", label: "Business link", required: false },
] as const;
...
function autoMap(p: Parsed): Record<FieldKey, number> {
  const map: Record<FieldKey, number> = { phone: -1, name: -1, business: -1, industry: -1, businessLink: -1 };
  ...
  if (p.hasHeader) {
    ...
    map.businessLink = lower.findIndex((h) => /business.*(link|url)|website|site|review|rating|gmb|maps|listing|profile/.test(h));
  }
  if (map.phone < 0) {
  ...
  const taken = new Set([map.phone, map.business, map.name, map.industry, map.businessLink].filter((i) => i >= 0));
...
export default function LeadImport({
  ...
  onImport: (
    rows: { name: string; business: string; phone: string; industry: string; businessLink: string }[],
    listName: string
  ) => Promise<{ added: number; updated: number; skipped: number } | null>;
  onDone: () => void;
}) {
  const [parsed, setParsed] = useState<Parsed | null>(null);
  const [map, setMap] = useState<Record<FieldKey, number>>({ phone: -1, name: -1, business: -1, industry: -1, businessLink: -1 });
  ...
  const doImport = async () => {
    ...
    const rows = parsed.rows
      .map((r) => ({
        ...
        businessLink: map.businessLink >= 0 ? r[map.businessLink] || "" : "",
      }))
