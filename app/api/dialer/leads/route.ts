...
export async function POST(request: NextRequest) {
  ...
  for (const row of rows) {
    ...
    const reviewLink = String(row?.reviewLink ?? "").trim().slice(0, 500);
    const state = stateOfAreaCode(areaCodeOf(phone));
    const result = (await q`
      INSERT INTO dialer_leads (phone, name, business, state, industry, list_name, review_link)
      VALUES (${phone}, ${name}, ${business}, ${state}, ${industry}, ${listName}, ${reviewLink})
      ON CONFLICT (phone) DO UPDATE SET
        ...
        review_link = CASE WHEN dialer_leads.review_link = '' THEN EXCLUDED.review_link ELSE dialer_leads.review_link END,
