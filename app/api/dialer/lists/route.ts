...
export async function PATCH(request: NextRequest) {
  ...
  // Renaming the unlisted group (from = "") must be explicit — a malformed
  // request must never silently grab every unlisted lead.
  if ((!from && body?.unlisted !== true) || !to) {
    return NextResponse.json({ error: "Need from and to" }, { status: 400 });
  }
...
export async function DELETE(request: NextRequest) {
  ...
  // Deleting the unlisted group (name = "") must be explicit — a malformed
  // request must never silently wipe every unlisted lead.
  if (!name && body?.unlisted !== true) {
    return NextResponse.json({ error: "Need a list name" }, { status: 400 });
  }
