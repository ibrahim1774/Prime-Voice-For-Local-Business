// US geographic area code → state (two-letter). Used for local-presence
// caller-ID matching: exact area code first, then any number in the same
// state, then the default line. Unknown codes just fall through to default.
...
const AREA_TO_STATE: Record<string, string> = {};
for (const [state, codes] of Object.entries(STATE_CODES)) {
  for (const code of codes) AREA_TO_STATE[code] = state;
}
...
export function stateOfAreaCode(code: string): string {
  return AREA_TO_STATE[code] || "";
}
