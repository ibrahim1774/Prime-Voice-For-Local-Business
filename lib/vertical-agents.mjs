...
export function buildVerticalAssistant(key, opts = {}) {
  ...
  const assistant = {
    ...
    // Copied verbatim from the live sample — this is what makes it answer
    // fast. waitSeconds 0 means no flat pause after the caller stops. The
    // waitFunction is livekit's confidence curve: x is the model's confidence
    // that the caller has finished, so a confident end waits ~20ms and a
    // trailing-off one waits longer. Do not flatten it to a constant.
    startSpeakingPlan: {
      waitSeconds: 0,
      ...
      smartEndpointingPlan: {
        provider: "livekit",
        waitFunction: "20 + 300 * sqrt(x) + 1500 * x^3",
      },
    },
  };
