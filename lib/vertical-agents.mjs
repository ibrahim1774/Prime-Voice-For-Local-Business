 ...
 *
 * Settings are inherited from the male sample voice in /api/create-demo and
 * must not be "improved":
 *   - sonic-2, NOT sonic-3. Sonic-3 ships a different voice set and dropped
 *     these classic ids, which makes the male voice fall back to a female
 *     default with no error.
 *   - emotion is an ARRAY. A plain string fails at runtime with "Only one
 *     emotion intensity level per emotion type is allowed".
 *   - chunkPlan feeds fuller phrases to the TTS so prosody flows instead of
...
// Cartesia voices (sonic-2) — the same pair every other demo uses.
export const VERTICAL_MALE_VOICE_ID = "9fa83ce3-c3a8-4523-accc-173904582ced";
export const VERTICAL_FEMALE_VOICE_ID = "db6b0ed5-d5d3-463d-ae85-518a07d3c2b4"; // Skylar
...
12. Match the caller's energy — upbeat with upbeat, calm and steady with stressed or frustrated callers.
...
12. Match the caller's energy — upbeat with upbeat, calm and steady with stressed or frustrated callers.
13. Use natural filler words occasionally ("sure thing", "absolutely", "of course", "gotcha", "no worries").
14. Inject light, appropriate wit when the conversation allows — warm and professional, never corporate. Never when someone is dealing with an emergency.
15. Show genuine enthusiasm when a caller wants to see a property.
16. NEVER speak stage directions, emotion labels, or bracketed/asterisked text (e.g. "[warm]", "[reassuring]", "(pause)", "*laughs*"). The voice reads those aloud verbatim, which sounds broken. Convey warmth and emotion ONLY through natural word choice, phrasing, and punctuation — never annotations.
17. When a caller describes an emergency, respond with genuine concern, using calmer and softer wording.
18. Sound relaxed and conversational, like a real person chatting — natural rhythm, contractions, and the occasional filler, never stiff or monotone.`,
  },
...
12. Match the caller's energy — upbeat with upbeat, calm and steady with stressed or frustrated callers.
13. Use natural filler words occasionally ("sure thing", "absolutely", "of course", "gotcha", "no worries").
14. Inject light, appropriate wit when the conversation allows — warm and professional, never corporate. Never while someone is standing in water.
15. Show genuine enthusiasm when a caller wants to book.
16. NEVER speak stage directions, emotion labels, or bracketed/asterisked text (e.g. "[warm]", "[reassuring]", "(pause)", "*laughs*"). The voice reads those aloud verbatim, which sounds broken. Convey warmth and emotion ONLY through natural word choice, phrasing, and punctuation — never annotations.
17. When a caller describes an emergency, respond with genuine concern, using calmer and softer wording.
18. Sound relaxed and conversational, like a real person chatting — natural rhythm, contractions, and the occasional filler, never stiff or monotone.`,
 ...
 *   end-of-call report here. Omit for web demos so no webhook URL reaches the
 *   browser bundle.
...
export function buildVerticalAssistant(key, opts = {}) {
  ...
  const voiceId =
    voiceGender === "female" ? VERTICAL_FEMALE_VOICE_ID : VERTICAL_MALE_VOICE_ID;
  ...
  const assistant = {
    ...
    // Respond fast — don't sit silent after the caller stops talking.
    startSpeakingPlan: {
      waitSeconds: 0.4,
